...
## 苏格拉底式引导模式
- [x] 设计引导问题和选项的数据结构
- [x] 实现后端 API：生成引导问题和选择题选项
- [x] 实现后端 API：验证学生选择并给出反馈（前端处理）
- [x] 创建前端引导模式界面（问题 + 选项按钮）
- [x] 实现选项选择和反馈显示逻辑
- [x] 添加“切换到引导模式”按钮
- [x] 实现引导模式进度追踪（第几个问题/总共几个问题）
- [ ] 编写引导模式的单元测试

## Bug 修复 - 引导模式错误
- [x] 修复点击引导模式按钮后的错误（添加错误处理）
- [x] 检查浏览器控制台错误日志
- [x] 将“引导模式”改名为“苏格拉底模式”

## Bug 修复 - LLM API 返回结构异常
- [ ] 添加调试日志查看 LLM 实际返回内容
- [ ] 修复 LLM API 调用逻辑
- [ ] 测试苏格拉底模式功能
...
## 题目详情页布局重构 v2
- [x] 添加答案图片显示区域（左侧最下方）
- [x] 解题步骤点击后显示两个选项："怎么得到这一步"和"下一步怎么思考"
- [x] 重新布局：左侧放题目、已知条件、解题步骤、答案；右侧放苏格拉底模式
- [x] 确保答案图片正确显示

## 题目文字显示优化
- [x] 修改数据库 schema 添加 problemTextEn 字段（英文题目）
- [x] 修改题目识别 API 同时提取中文和英文题目文字
- [x] 修改题目详情页：先显示中文题目、英文题目，然后才显示题目图片
- [x] 更新现有题目数据，重新识别提取英文题目

## 修正数学符号显示
- [x] 修正题目 30002 的中文文字，使用正确的 LaTeX 格式（用 $ 包裹数学符号）
- [x] 修正题目 30002 的英文文字，使用正确的 LaTeX 格式（用 $ 包裹数学符号）
- [x] 测试数学公式渲染效果

## 优化题目识别系统
- [x] 优化 extractSteps API 的 AI 提示词，明确要求提取 LaTeX 格式的数学公式
- [x] 在管理员上传页面添加可编辑的文本框，显示识别的中英文题目
- [x] 允许管理员手动修改识别结果后再提交
- [x] 测试新的识别和编辑功能

## 优化已知条件说明文字
- [x] 修改题目详情页的“已知条件”栏目说明文字
- [x] 说明如何使用这些条件开始解题、如何转化为数学等式
- [x] 测试新的说明文字显示效果
